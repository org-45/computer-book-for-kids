...
export default function ChapterNavigation({ chapters }: ChapterNavigationProps) {
  ...
  return (
      <div className="h-full">
          ...
          <div className="hidden md:block w-full bg-gray-100 p-4 h-full">
              <CardContent className="flex-grow overflow-y-auto" style={{height:"95%"}}>
              ...
              <div className="p-4 relative">
                  <div className="flex absolute justify-between inset-x-0 bottom-0">
                      <Button variant="default" onClick={prevPage} disabled={currentPage === 1} className="text-sm">
                          Previous
                      </Button>
                      <Button
                          variant="default"
          ...
          {isModalOpen && (
              <div
                  className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 h-full"
                  onClick={closeModal}>
                  <div
                      className="bg-white w-full max-w-md h-4/5 max-h-screen overflow-y-auto p-6 rounded-lg relative z-60"
                      ...
                      <CardContent className="flex-grow overflow-y-auto h-4/5">
                          <ul className="space-y-2">
                              {currentChapters.map(chapter => (
                                  <li key={chapter.id}>
                                      <Link
